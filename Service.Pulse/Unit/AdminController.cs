    ...
    public class AdminController : Controller
    {
        ...
        public override void OnActionExecuting(ActionExecutingContext filterContext)
        {
            ...
            if (_loginUser.IsLogin)
            {
                ...
                //判定是否为后台账号
                int no = _loginUser.userNo;
                var adminUserService = App.GetService<IAdminUserService>();
                var adminInfo = adminUserService.GetAdminUserInfoByNoAsyc(no);
                if (adminInfo == null)
                {
                    filterContext.Result = new RedirectResult(UnitTool.UrlToSid("/Index/Index", _loginUser.OnLineSid));
                    return;
                }
